...
def main():
    ...
    print("\nSedang melatih Model LightGBM dengan data hasil SMOTE...")
    lgbm_model = LGBMClassifier(
        n_estimators=100,
        random_state=42,
        verbosity=-1  # Sembunyikan warning bawaan
    )
    ...
    print("\n" + "="*50)
    print("HASIL EVALUASI MODEL: LIGHTGBM CLASSIFIER")
    print("="*50)
    ...
    try:
        ...
        plt.barh(feature_df['Fitur'], feature_df['Tingkat Kepentingan'], color='lightgreen')
        plt.title('Fitur Paling Berpengaruh (LightGBM)')
        plt.tight_layout()
        output_image = os.path.join(script_dir, 'Feature_Importance_Fraud_LightGBM.png')
        plt.savefig(output_image)
        print(f"\n[SUCCESS] Gambar Feature Importance LightGBM disimpan sebagai '{output_image}'")
